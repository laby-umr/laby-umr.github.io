---
sidebar_position: 1
title: JavaScript核心基础与现代特性
description: 深入解析JavaScript语言核心、数据类型、函数式编程与面向对象编程实践
authors: [Laby]
tags: [JavaScript, ES6+, 数据类型, 函数式编程, 面向对象, 异步编程, 原型链]
last_update:
  date: 2025-08-14
  author: Laby
---
...
```mermaid
graph TB
    subgraph "JavaScript函数类型"
        A[函数声明] --> A1[function declaration]
        A --> A2[提升到作用域顶部]
        A --> A3[可在声明前调用]
        
        B[函数表达式] --> B1[function expression]
        B --> B2[不会提升]
        B --> B3[只能在定义后调用]
        
        C[箭头函数] --> C1[arrow function]
        C --> C2[简洁语法]
        C --> C3[词法this绑定]
