---
sidebar_position: 7
title: OpenTwins Windows 本地部署实战指南
description: OpenTwins 数字孪生平台 Windows 本地环境完整部署实战记录
tags: [数字孪生, OpenTwins, Windows部署, 本地部署, Kubernetes, Minikube, Docker]
---
...
# 检查 Docker 版本
docker --version
# 输出示例: Docker version 28.5.1, build e180ab8
...
# 输出示例: Client Version: v1.34.1
```

### 2.3 安装 Chocolatey（可选但推荐）
...
```powershell
helm version
# 输出示例: version.BuildInfo{Version:"v3.18.6"...}
```
...
:::tip 启动时间
首次启动 Minikube 需要下载 Kubernetes 镜像，可能需要 5-10 分钟，请耐心等待。
:::

预期输出：

```
* Microsoft Windows 11 Pro 10.0.26100 上的 minikube v1.37.0
...
```powershell
# 配置 kubectl 使用 minikube 上下文
kubectl config use-context minikube

# 验证集群连接
kubectl cluster-info
# 输出: Kubernetes control plane is running at https://127.0.0.1:xxxxx
...
:::info 安装说明
- `--wait`: 等待所有 Pod 就绪
- `--timeout 20m`: 设置超时时间为 20 分钟（首次安装需要拉取镜像）
- `--dependency-update`: 自动更新依赖的 Chart
:::
...
2. **镜像拉取阶段（5-15分钟）**
   - 下载各组件的 Docker 镜像
   - 镜像大小总计约 2-3GB
   - 状态显示为 `ContainerCreating` 或 `Init`
...
- `Running`: 容器正在运行
- `CrashLoopBackOff`: 容器启动失败，正在重试（Telegraf 在等待 InfluxDB 时会出现，这是正常的）
:::
...
由于 Windows 上 Minikube 的网络限制，无法直接通过 Minikube IP 访问服务。我们需要使用**端口转发**的方式。
...
```powershell
# Grafana（数据可视化）
kubectl port-forward svc/opentwins-grafana 3002:80
...
:::tip 提示
端口转发命令会占用当前终端窗口，需要保持窗口打开。建议为每个服务开启一个新的 PowerShell 窗口。
:::
...
Write-Host "启动 OpenTwins 端口转发" -ForegroundColor Green
Write-Host "======================================" -ForegroundColor Green
Write-Host ""

# 启动端口转发（后台运行）
Write-Host "正在启动端口转发..." -ForegroundColor Yellow
Write-Host ""
...
Write-Host "  查看后台任务: Get-Job" -ForegroundColor Gray
Write-Host "  停止所有转发: Get-Job | Stop-Job; Get-Job | Remove-Job" -ForegroundColor Gray
Write-Host ""
Write-Host "提示: 保持此窗口打开以维持端口转发" -ForegroundColor Yellow
Write-Host "      按 Ctrl+C 可以停止所有转发" -ForegroundColor Yellow
Write-Host ""

# 显示后台任务状态
Write-Host "后台任务状态:" -ForegroundColor Yellow
Get-Job | Format-Table -AutoSize

Write-Host ""
Write-Host "正在监控端口转发状态... (按 Ctrl+C 停止)" -ForegroundColor Gray
Write-Host ""
...
try {
    while ($true) {
        ...
        if ($failedJobs) {
            Write-Host "警告: 有端口转发任务失败" -ForegroundColor Red
            $failedJobs | Format-Table -AutoSize
        }
    }
} finally {
    Write-Host ""
    Write-Host "正在停止所有端口转发..." -ForegroundColor Yellow
    Get-Job | Stop-Job
    Get-Job | Remove-Job
    Write-Host "已停止所有端口转发" -ForegroundColor Green
}
```
...
Eclipse Ditto 提供了完整的数字孪生管理 API。
...
OpenTwins 的扩展 API 接口文档。

:::warning 注意
根路径 `http://localhost:8081/` 会显示 "Cannot GET /"，这是**正常现象**。
...
1. **使用 MQTT Explorer（推荐）**
   - 下载: [http://mqtt-explorer.com/](http://mqtt-explorer.com/)
   - 安装后配置连接:
     - Protocol: `mqtt://`
     - Host: `localhost`
     - Port: `1883`
...
2. **使用命令行工具**
   ...
   # 发布消息
   mosquitto_pub -h localhost -p 1883 -t "test/topic" -m "Hello OpenTwins"
   ```
...
3. **使用 Python**
   ...
   client.publish("test/topic", "Hello OpenTwins!")
   client.disconnect()
   ```
...
**原因**: 这是**正常现象**，不是错误！Extended API 的根路径没有配置任何内容。
...
2. **使用 MQTT 客户端连接**：
   - 下载 [MQTT Explorer](http://mqtt-explorer.com/)
   - 或使用命令行：`mosquitto_pub -h localhost -p 1883 -t "test" -m "hello"`

#### 问题 3: Grafana 端口 3002 已被占用

**现象**: 启动端口转发时报错：
```
Error listen tcp4 127.0.0.1:3002: bind: Only one usage of each socket address
```

**原因**: 端口 3002 已经被其他程序占用，或者已经有一个端口转发在运行。
...
2. **使用不同的本地端口**：
   ```powershell
   kubectl port-forward svc/opentwins-grafana 3003:80
...
### 推荐工具
- [MQTT Explorer](http://mqtt-explorer.com/) - MQTT 客户端（图形界面）
