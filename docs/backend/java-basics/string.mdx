---
sidebar_position: 3
title: "Java String类详解"
description: "全面介绍Java String类的特性、操作方法与最佳实践，包含深入的内存机制分析和性能优化建议"
authors: [Laby]
last_update:
  date: 2025-08-07
  author: Laby
...
```mermaid
graph LR
    A["String s1 = 'Hello'"] --> B["内存中的Hello对象"]
    C["s1.concat(' World')"] --> D["创建新对象Hello World"]
    E["s1 = s1.concat(' World')"] --> F["s1引用更新为新对象"]
    B --> B1["原字符串不变"]
```
...
graph TD
    ...
    D["String s2 = 'Hello'"] --> B
    B -- "重用" --> C
    E["String s3 = new String('Hello')"] --> F["堆内存"]
    F -- "创建" --> G["新的Hello对象"]
    H["s3.intern()"] --> B
    B -- "返回引用" --> C
```
...
classDiagram
    ...
    note for String "Java 9+实现"
    note for StringJava8 "Java 8实现"
```
...
```mermaid
flowchart LR
    A["原始字符串"] -->|"trim()"| B["去除前后空白"]
    A -->|"toUpperCase()"| C["转为大写"]
    A -->|"toLowerCase()"| D["转为小写"]
    A -->|"replace()"| E["替换字符或子串"]
    A -->|"replaceAll()"| F["正则表达式替换"]
    A -->|"concat()"| G["字符串连接"]
    G -.->|"+"| H["使用+运算符"]
    G -.->|"String.join()"| I["使用静态方法"]
```
...
graph TD
    ...
    C --> C1["StringBuilder<br/>非线程安全<br/>性能最佳"]
    C --> C2["StringBuffer<br/>线程安全<br/>性能较低"]
...
classDiagram
    ...
    note for StringBuilder "非线程安全，性能最佳"
    note for StringBuffer "线程安全，方法都有synchronized修饰"
```
