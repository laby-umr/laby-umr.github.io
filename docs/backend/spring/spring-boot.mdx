...
```mermaid
flowchart TD
    A[配置文件] --> B[通用配置<br>application.yml/properties]
    A --> C[特定环境配置]
    C --> D[开发环境<br>application-dev.yml]
    C --> E[测试环境<br>application-test.yml]
    C --> F[生产环境<br>application-prod.yml]
    B --> G[应用程序]
    D --> G
    E --> G
    F --> G
    H[Profile激活方式] --> I[命令行参数]
    H --> J[环境变量]
    H --> K[JVM系统属性]
    H --> L[配置文件中指定]
...
sequenceDiagram
    ...
    App->>SA: 调用SpringApplication.run()
    SA->>SA: 创建SpringApplication对象
    Note over SA: 推断应用类型<br>设置初始化器<br>设置监听器
    SA->>AL: 发布ApplicationStartingEvent
    SA->>SA: 准备环境
    SA->>AL: 发布ApplicationEnvironmentPreparedEvent
    SA->>AC: 创建ApplicationContext
    Note over AC: 根据应用类型创建不同的上下文
    SA->>AC: 准备上下文
    SA->>AL: 发布ApplicationContextInitializedEvent
    SA->>AC: 加载Bean定义
    SA->>AL: 发布ApplicationPreparedEvent
    SA->>AC: 刷新上下文
    AC->>AB: 处理自动配置
    AC->>AC: 实例化单例Bean
    SA->>AL: 发布ApplicationStartedEvent
    SA->>SA: 调用ApplicationRunner和CommandLineRunner
    SA->>AL: 发布ApplicationReadyEvent
```
...
```mermaid
graph TD
    A[Spring Boot事件] --> B[ApplicationStartingEvent]
    ...
    B --> B1["应用启动时<br>框架启动开始"]
    C --> C1["环境准备就绪<br>上下文创建之前"]
    D --> D1["上下文已创建并准备<br>但尚未加载Bean定义"]
    E --> E1["上下文已刷新<br>但尚未启动"]
    F --> F1["上下文已刷新<br>Runner执行前"]
    G --> G1["应用已启动<br>Runner已执行"]
    H --> H1["启动失败<br>异常处理"]
...
```mermaid
graph TD
    A[刷新ApplicationContext] --> B[onRefresh方法]
    B --> C[创建WebServer]
    C --> D[获取Servlet上下文]
    D --> E[启动内嵌容器]
    E --> F[发布嵌入式容器启动事件]
...
```mermaid
graph TD
    A[刷新ApplicationContext] --> B[执行BeanFactoryPostProcessor]
    B --> C["处理@Configuration类"]
    C --> D["处理@Import注解"]
    D --> E["处理@EnableAutoConfiguration"]
