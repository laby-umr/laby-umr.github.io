...
last_update:
  date: 2025-08-07
  author: Laby
tags: [微服务, 服务发现, Eureka, Consul, Nacos, 负载均衡]
---
...
graph TB
    ...
    Provider1 -->|注册| Registry1
    Provider2 -->|注册| Registry2
    Provider3 -->|注册| Registry3
    Provider4 -->|注册| Registry1
    Provider5 -->|注册| Registry2
    
    Registry1 <--> Registry2
    Registry2 <--> Registry3
    Registry3 <--> Registry1
    
    Consumer1 -->|发现| Registry1
    Consumer2 -->|发现| Registry2
    Consumer3 -->|发现| Registry3
...
<Tabs>
<TabItem value="round-robin" label="轮询算法">
...
<TabItem value="weighted" label="权重算法">
...
public class LeastConnectionLoadBalancer implements LoadBalancer {
    ...
    public ServiceInstance choose(String serviceId, List<ServiceInstance> instances) {
        return instances.stream()
            .min(Comparator.comparingInt(instance -> 
                connectionCounts.getOrDefault(instance.getInstanceId(), 
...
<Tabs>
<TabItem value="http-check" label="HTTP检查">
...
public class HttpHealthCheck {
    ...
    public HttpHealthCheck(String healthCheckUrl, int timeout) {
...
<TabItem value="tcp-check" label="TCP检查">
...
public class TcpHealthCheck {
    ...
    public TcpHealthCheck(int port, int timeout) {
        this.port = port;
        this.timeout = timeout;
    }
...
<TabItem value="heartbeat" label="心跳检查">
...
public class HeartbeatHealthCheck {
    ...
    public HeartbeatHealthCheck(long heartbeatTimeout) {
...
<Tabs>
<TabItem value="client-side" label="客户端发现">
...
<TabItem value="server-side" label="服务端发现">
