...
graph TD
    ...
    B --> B1["key拼接 (user.getName() + '|' + user.getAge())"]
    B --> B2["CompositeKey对象 (name, department, location)"]
    B --> B3["多字段提取器"]
...
graph TD
    ...
    B --> B1["需要顺序: LinkedHashSet"]
    B --> B2["高性能: HashSet"]
    B --> B3["自动排序: TreeSet"]
    B --> B4["多字段: CompositeKey"]
